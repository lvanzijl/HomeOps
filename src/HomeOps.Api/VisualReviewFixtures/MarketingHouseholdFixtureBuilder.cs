...
internal static class MarketingHouseholdFixtureBuilder
{
    ...
    public static void Add(HomeOpsDbContext db, MarketingScene scene)
    {
        ...
        AddAgenda(db, scene);
        AddTasks(db, scene);
        AddShopping(db, scene);
        AddMotivation(db, scene);
    }

    private static Guid Id(int i) => Guid.Parse($"88000000-0000-0000-0000-{i:000000000000}");

    private static void AddFamily(HomeOpsDbContext db) => db.FamilyMembers.AddRange(
        Member("dad", "Dad", FamilyMemberKind.Adult, "#93c5fd", "D", null, new() { HeadVariant = "oval", HairStyle = "softCrop", HairColor = "hairCocoa", ClothingStyle = "hoodie", ClothingColor = "shirtSky", Accessory = "none", AccessoryColor = "accessoryLilac" }),
        Member("mom", "Mom", FamilyMemberKind.Adult, "#86efac", "M", null, new() { HeadVariant = "oval", HairStyle = "longSoft", HairColor = "hairChestnut", ClothingStyle = "sweater", ClothingColor = "shirtMint", Accessory = "none", AccessoryColor = "accessoryLilac" }),
        Member("thomas", "Thomas", FamilyMemberKind.Child, "#60a5fa", "T", new DateOnly(2020, 4, 12), new() { HeadVariant = "round", HairStyle = "shortMessy", HairColor = "hairChestnut", ClothingStyle = "tShirt", ClothingColor = "shirtSky", Accessory = "star", AccessoryColor = "accessoryCoral" }),
        Member("robin", "Robin", FamilyMemberKind.Child, "#fde68a", "R", new DateOnly(2024, 2, 8), new() { HeadVariant = "round", HairStyle = "curlyCloud", HairColor = "hairCocoa", ClothingStyle = "overall", ClothingColor = "shirtSun", Accessory = "none", AccessoryColor = "accessoryLilac" }));

    private static FamilyMember Member(string id, string name, FamilyMemberKind kind, string color, string initials, DateOnly? dob, AvatarV2Config avatar) => new() { Id = id, HouseholdId = SeedHousehold.Id, Name = name, DisplayColor = color, Initials = initials, MemberKind = kind, DateOfBirth = dob, AvatarV2Config = avatar, CreatedUtc = AnchorUtc, UpdatedUtc = AnchorUtc };

    private static void AddCalendarSource(HomeOpsDbContext db) => db.EventSources.Add(new() { Id = CalendarSourceId, HouseholdId = SeedHousehold.Id, Name = "Van Zijl Family Calendar", SourceType = "manual", IsWritable = true, CreatedUtc = AnchorUtc, UpdatedUtc = AnchorUtc });

    private static void AddAgenda(HomeOpsDbContext db, MarketingScene scene)
    {
    ...
    private static void AddTasks(HomeOpsDbContext db, MarketingScene scene)
    {
        var weeklyResetComplete = scene is MarketingScene.WeeklyReset;
        AddTask(db, 101, "Zwemtas klaarzetten", "dad", CanonicalTuesday, weeklyResetComplete || scene is MarketingScene.Motivation, TaskOwnershipKind.FamilyMember);
        AddTask(db, 102, "Fruitbakje en drinkbeker school", "mom", CanonicalTuesday, weeklyResetComplete, TaskOwnershipKind.FamilyMember);
        AddTask(db, 103, "Cadeau voor Noor inpakken", "thomas", CanonicalTuesday, weeklyResetComplete, TaskOwnershipKind.FamilyMember);
        AddTask(db, 104, "Reservekleren daycare tas", "mom", CanonicalTuesday, weeklyResetComplete, TaskOwnershipKind.FamilyMember);
        AddTask(db, 105, "Pasta saus uit vriezer halen", "dad", CanonicalTuesday, scene is not MarketingScene.Tasks, TaskOwnershipKind.FamilyMember);
        AddTask(db, 106, "Football shirt wassen", "dad", CanonicalTuesday.AddDays(1), weeklyResetComplete, TaskOwnershipKind.FamilyMember);
        AddTask(db, 107, "Bibliotheekboeken zoeken", "thomas", CanonicalTuesday.AddDays(1), weeklyResetComplete, TaskOwnershipKind.FamilyMember);
        AddTask(db, 108, "Daycare forms checken", "mom", CanonicalTuesday.AddDays(1), false, TaskOwnershipKind.FamilyMember);
        AddTask(db, 109, "Cadeauzakje en kaart voor Noor kopen", "mom", new DateOnly(2026, 6, 19), weeklyResetComplete, TaskOwnershipKind.FamilyMember);
        AddTask(db, 110, "Father's Day pancake ingredients checken", null, new DateOnly(2026, 6, 20), weeklyResetComplete, TaskOwnershipKind.SharedHousehold);
        AddTask(db, 111, "Picknickkleed wassen", "dad", new DateOnly(2026, 6, 25), false, TaskOwnershipKind.FamilyMember);
    }

    private static void AddTask(HomeOpsDbContext db, int id, string title, string? member, DateOnly due, bool complete, TaskOwnershipKind ownership) => db.HouseholdTasks.Add(new() { Id = Id(id), HouseholdId = SeedHousehold.Id, Title = title, DueDate = due, OwnershipKind = ownership, FamilyMemberId = member, IsCompleted = complete, CompletedUtc = complete ? AnchorUtc.AddHours(id - 100) : null, CreatedUtc = AnchorUtc.AddDays(-4), UpdatedUtc = AnchorUtc });

    private static void AddShopping(HomeOpsDbContext db, MarketingScene scene)
    {
    ...
    private static void AddMotivation(HomeOpsDbContext db, MarketingScene scene)
    {
        ...
        AddMoment(db, 401, "Thomas helped Robin find bunny", "Thomas helped Robin find her bunny before daycare, even though his own shoes were still missing.", "thomas", HelpfulMomentTags.Kindness, -1);
        AddMoment(db, 402, "Mom packed the daycare bag early", "Dad appreciated Mom for packing the daycare bag before breakfast when Robin had a hard morning.", "mom", HelpfulMomentTags.Teamwork, -2);
        AddMoment(db, 403, "Towel and goggles by the door", "Mom appreciated Dad for keeping swimming calm by putting the towel and goggles near the door.", "dad", HelpfulMomentTags.Responsibility, -3);
        AddMoment(db, 404, "Grocery bags back on the hook", "Mom appreciated Thomas for putting the grocery bags back on the hallway hook.", "thomas", HelpfulMomentTags.Initiative, -4);
        if (complete || scene is MarketingScene.Motivation) AddMoment(db, 405, "Robin tried shoes by herself", "Dad appreciated Robin for trying to put her shoes by the mat, even though they were on the wrong feet.", "robin", HelpfulMomentTags.Routine, -5);
    }

    private static void AddMoment(HomeOpsDbContext db, int id, string title, string description, string member, string tag, int days) => db.HelpfulMoments.Add(new() { Id = Id(id), HouseholdId = SeedHousehold.Id, FamilyMemberId = member, Title = title, Description = description, RecognitionTag = tag, CreatedUtc = AnchorUtc.AddDays(days) });
}
