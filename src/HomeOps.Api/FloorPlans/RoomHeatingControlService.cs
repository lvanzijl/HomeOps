...
public sealed class RoomHeatingControlService(HomeOpsDbContext db, TimeProvider clock, IRoomHeatingControlProvider provider)
{
    ...
    public async Task<RoomHeatingControlCapabilityDto?> GetCapability(Guid roomId, CancellationToken ct)
    {
        ...
        var latest = await db.RoomHeatingCommands.AsNoTracking().Where(c => c.RoomId == roomId).OrderByDescending(c => c.RequestedUtc).Select(c => new RoomHeatingCommandSummaryDto(c.Id, c.Action, c.Status, c.RequestedUtc, c.UpdatedUtc)).FirstOrDefaultAsync(ct);
