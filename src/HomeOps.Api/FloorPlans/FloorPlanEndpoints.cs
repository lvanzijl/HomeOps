...
public static class FloorPlanEndpoints
{
    ...
    private static void MapRoomEndpoints(IEndpointRouteBuilder app)
    {
        ...
        rooms.MapDelete("/{roomId:guid}", async (Guid roomId, HomeOpsDbContext db, CancellationToken ct) => { var room = await db.Rooms.FirstOrDefaultAsync(r => r.Id == roomId && r.HouseholdId == SeedHousehold.Id, ct); if (room is null) return Results.NotFound(); var overlays = await db.RoomOverlays.Where(o => o.RoomId == room.Id && o.HouseholdId == SeedHousehold.Id).ToListAsync(ct); db.RoomOverlays.RemoveRange(overlays); db.Rooms.Remove(room); await CompactRoomOrders(db, room.FloorId, ct); await db.SaveChangesAsync(ct); return Results.NoContent(); }).WithName("DeleteRoom").Produces(204).Produces(404);
    }
    ...
    private static async Task<IResult> MoveRoom(Guid roomId, MoveRoomRequest req, HomeOpsDbContext db, CancellationToken ct)
    {
        ...
        var source = room.FloorId; room.FloorId = dest.Id; room.SortOrder = await NextRoomOrder(db, dest.Id, ct); room.UpdatedUtc = DateTimeOffset.UtcNow; var movedOverlays = await db.RoomOverlays.Where(o => o.RoomId == room.Id && o.HouseholdId == SeedHousehold.Id && o.State != RoomOverlayState.Archived).ToListAsync(ct); foreach (var overlay in movedOverlays) { overlay.State = RoomOverlayState.NeedsReview; overlay.UpdatedUtc = room.UpdatedUtc; } await CompactRoomOrders(db, source, ct); await db.SaveChangesAsync(ct); return Results.Ok(ToRoomDto(room));
    }
    private static async Task<IResult> ArchiveRoom(Guid roomId, HomeOpsDbContext db, CancellationToken ct) { var room = await db.Rooms.FirstOrDefaultAsync(r => r.Id == roomId && r.HouseholdId == SeedHousehold.Id, ct); if (room is null) return Results.NotFound(); room.IsArchived = true; room.IsEnabled = false; room.ArchivedUtc = DateTimeOffset.UtcNow; room.UpdatedUtc = room.ArchivedUtc.Value; var overlays = await db.RoomOverlays.Where(o => o.RoomId == room.Id && o.HouseholdId == SeedHousehold.Id && o.State == RoomOverlayState.Trusted).ToListAsync(ct); foreach (var overlay in overlays) { overlay.State = RoomOverlayState.NeedsReview; overlay.UpdatedUtc = room.UpdatedUtc; } await CompactRoomOrders(db, room.FloorId, ct); await db.SaveChangesAsync(ct); return Results.NoContent(); }
