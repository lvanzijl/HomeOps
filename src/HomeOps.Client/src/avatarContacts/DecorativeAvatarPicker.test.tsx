...
import {
  ...
  resolveDecorativeAvatar,
} from "./DecorativeAvatarPicker";
...
describe("DecorativeAvatarPicker", () => {
  it("keeps manual groups, suggestions, and clear behavior shared for consumers", async () => {
    ...
    render(
      <DecorativeAvatarPicker
        ...
        label="Decoratieve avatar"
        onChange={onChange}
        suggestionText="Grandma gift"
        value={null}
      />,
    );
    ...
    expect(
      within(select).getByRole("option", { name: "Geen avatar" }),
    ).not.toBeNull();
    expect(
      within(select).getByRole("group", { name: "Voorgesteld" }),
    ).not.toBeNull();
    expect(
      within(select).getByRole("group", { name: "Gezinsleden" }),
    ).not.toBeNull();
    expect(
      within(select).getByRole("group", { name: "Gedeelde bekenden" }),
    ).not.toBeNull();
  ...
  it("resolves FamilyMember and KnownPerson identities without Shopping ownership", () => {
    expect(
      resolveDecorativeAvatar(
        { referenceType: "familyMember", referenceId: "riley" },
        members,
        knownPeople,
      )?.kind,
    ).toBe("familyMember");
    expect(
      resolveDecorativeAvatar(
        { referenceType: "knownPerson", referenceId: "known-1" },
        members,
        knownPeople,
      )?.kind,
    ).toBe("knownPerson");
    expect(
      resolveDecorativeAvatar(
        { referenceType: "knownPerson", referenceId: "missing" },
        members,
        knownPeople,
      ),
    ).toBeNull();
