...
function TaskGroup({ groupTitle, members, tasks, onUpdate }: { groupTitle: string; members: readonly FamilyMember[]; tasks: readonly HouseholdTask[]; onUpdate(id: string, action: 'complete' | 'reopen'): void }) {
  return <section className="task-group"><h4>{groupTitle}</h4>{tasks.length === 0 ? <p className="shopping-empty">No tasks.</p> : <ul className="task-list">{tasks.map((task) => <li className="task-item" key={task.id}><div><strong>{task.title}</strong><span>{formatOwner(task, members)}{task.dueDate ? ` · Due ${task.dueDate}` : ''}</span></div><button onClick={() => onUpdate(task.id, task.isCompleted ? 'reopen' : 'complete')} type="button">{task.isCompleted ? 'Reopen' : 'Complete'}</button></li>)}</ul>}</section>;
}
