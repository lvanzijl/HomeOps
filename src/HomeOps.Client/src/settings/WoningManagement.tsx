...
type Dialog = { kind: "floor"; floor?: Floor } | { kind: "room"; room?: Room } | { kind: "climate"; room: Room } | { kind: "move"; room: Room } | { kind: "confirm"; action: string; title: string; body: string; run: () => Promise<void> } | null;

interface FloorCardProps { floor: Floor; selected: boolean; busy: boolean; onSelect: () => void; onEdit: () => void; onArchive: () => void; onDelete: () => void; onTop: () => void; onUp: () => void; onDown: () => void; onBottom: () => void; }
interface RoomCardProps { room: Room; config?: RoomClimateConfiguration | null; busy: boolean; onClimate: () => void; onEdit: () => void; onMove: () => void; onArchive: () => void; onDelete: () => void; onTop: () => void; onUp: () => void; onDown: () => void; onBottom: () => void; }
interface FloorFormProps { floor?: Floor; busy: boolean; onCancel: () => void; onSave: (name: string) => void | Promise<void>; }
interface RoomFormProps { room?: Room; members: readonly FamilyMember[]; busy: boolean; onCancel: () => void; onSave: (name: string, type: RoomType, member?: string) => void | Promise<void>; }
interface MoveFormProps { room: Room; floors: Floor[]; busy: boolean; onCancel: () => void; onMove: (floorId: string) => void | Promise<void>; }
interface ConfirmDialogProps { dialog: Extract<NonNullable<Dialog>, { kind: "confirm" }>; busy: boolean; onCancel: () => void; }
interface DialogProps { title: string; onCancel: () => void; children: ReactNode; }
interface ActionsProps { busy: boolean; onCancel: () => void; save: string; onSave?: () => void | Promise<void>; }
interface ArchivedProps { title: string; description?: string; children: ReactNode; }
...
export function WoningManagement({ members = [] }: Props) {
  ...
  const [floorPlanRevision, setFloorPlanRevision] = useState(0);
  const [replacementReviewOpen, setReplacementReviewOpen] = useState(false);
  ...
  async function reloadAll(preferredFloorId?: string) {
    ...
    try {
      ...
      setError(getFriendlyWoningError(err, "Woninggegevens inclusief klimaatinstellingen laden lukt niet."));
  ...
  async function refreshFloorPlan() {
    await reloadAll(selectedFloor?.id);
    setFloorPlanRevision((current) => current + 1);
  }
  ...
  if (replacementReviewOpen && selectedFloor) return <section className="woning-subworkspace" aria-label={`Vervangingscontrole voor ${selectedFloor.name}`}><header><div><p className="widget-type">Woning · Plattegrond vervangen</p><h3>{selectedFloor.name}</h3></div><button onClick={() => setReplacementReviewOpen(false)}>Terug naar Woning</button></header><FloorPlanReplacementReview key={`${selectedFloor.id}-${floorPlanRevision}`} floor={selectedFloor} rooms={activeRooms} activeAsset={selectedAsset ?? null} onRefresh={refreshFloorPlan} /></section>;
  ...
  return <section className="woning-workspace" aria-label="Woning beheren">
    ...
    <div className="woning-grid">
      ...
      <main className="woning-panel woning-room-panel">
        <div className="woning-panel-head"><div><h4>{selectedFloor?.name ?? "Geen verdieping geselecteerd"}</h4><p>{setupStatus(activeRooms.length, assetStatus(selectedAsset))}</p></div><button disabled={!selectedFloor} onClick={() => setDialog({ kind: "room" })}>Kamer toevoegen</button></div>
        <div className="woning-summary-strip"><span>{assetStatus(selectedAsset)}</span><span>{activeRooms.length ? "Bruikbaar" : "Niet gestart"}</span><span>{isUsableActiveAsset(selectedAsset) ? "Actieve veilige plattegrond" : "Uploaden beschikbaar"}</span></div>
        <div className="woning-room-regions">
          <div className="woning-scroll-list" role="list" aria-label="Kamers op geselecteerde verdieping">{activeRooms.map((room, index) => <RoomCard key={room.id} room={room} config={climate[room.id ?? ""]} onClimate={() => setDialog({ kind: "climate", room })} onEdit={() => setDialog({ kind: "room", room })} onMove={() => setDialog({ kind: "move", room })} onArchive={() => setDialog({ kind: "confirm", title: "Kamer archiveren", action: "Archiveren", body: "De kamer verdwijnt uit de actieve lijst, maar blijft herstelbaar.", run: () => runAction(() => archiveRoom(room.id ?? ""), "Kamer is gearchiveerd.", "Kamer archiveren lukt niet.") })} onDelete={() => setDialog({ kind: "confirm", title: "Kamer verwijderen", action: "Verwijderen", body: "Verwijderen kan alleen als de backend dit toestaat. Gebruik archiveren als je de kamer later wilt herstellen.", run: () => runAction(() => deleteRoom(room.id ?? ""), "Kamer is verwijderd.", "Kamer verwijderen lukt niet.") })} onTop={() => shiftRoom(room, 0)} onUp={() => shiftRoom(room, index - 1)} onDown={() => shiftRoom(room, index + 1)} onBottom={() => shiftRoom(room, activeRooms.length - 1)} busy={busy} />)}</div>
          <div className="woning-room-secondary" aria-label="Aanvullende woninginstellingen">
            {selectedFloor && activeRooms.length === 0 ? <div className="woning-empty"><p>Nog geen kamers op deze verdieping.</p><button onClick={() => setDialog({ kind: "room" })}>Kamer toevoegen</button></div> : null}
            <HomeAssistantClimateSettings rooms={activeRooms} onSelectRoom={(roomId) => { document.getElementById(`room-${roomId}`)?.focus(); }} />
            <div className="woning-plan-action"><div><h4>Plattegrond</h4><p>{isUsableActiveAsset(selectedAsset) ? `${selectedAsset?.originalFilename ?? "Plattegrond"} is actief voor ${selectedFloor?.name}.` : `Er is nog geen actieve plattegrond voor ${selectedFloor?.name ?? "deze verdieping"}.`}</p></div><div className="woning-actions"><button disabled={!selectedFloor || busy} onClick={() => setFloorPlanUploadOpen(true)}>{isUsableActiveAsset(selectedAsset) ? "Nieuwe plattegrond uploaden" : "Plattegrond uploaden"}</button>{isUsableActiveAsset(selectedAsset) ? <button onClick={() => setOverlayEditorOpen(true)}>Kamergrenzen tekenen</button> : null}</div></div>
            {selectedFloor ? <FloorPlanReplacementReview key={`${selectedFloor.id}-${floorPlanRevision}`} floor={selectedFloor} rooms={activeRooms} activeAsset={selectedAsset ?? null} onRefresh={refreshFloorPlan} embedded onOpenWorkspace={() => setReplacementReviewOpen(true)} /> : null}
            {archivedRooms.length ? <Archived title="Gearchiveerde kamers" description="Herstel een kamer voordat je de klimaatinstellingen kunt wijzigen.">{archivedRooms.map((room) => <button key={room.id} onClick={() => runAction(() => restoreRoom(room.id ?? "").then(() => undefined), "Kamer is hersteld.", "Kamer herstellen lukt niet.")}>Herstellen: {room.name}</button>)}</Archived> : null}
          </div>
        </div>
      </main>
    </div>
    ...
    {dialog?.kind === "climate" ? <RoomClimateConfigurationDialog room={dialog.room} configuration={climate[dialog.room.id ?? ""]} onClose={() => setDialog(null)} onSaved={(saved) => { setClimate((current) => ({ ...current, [dialog.room.id ?? ""]: saved })); setMessage(`Klimaatinstellingen voor ${dialog.room.name} zijn opgeslagen.`); }} /> : null}
    ...
    {floorPlanUploadOpen && selectedFloor ? <FloorPlanUploadDialog floor={selectedFloor} activeAsset={selectedAsset ?? null} onClose={() => setFloorPlanUploadOpen(false)} onChanged={refreshFloorPlan} onOpenEditor={() => setOverlayEditorOpen(true)} onOpenReplacementReview={() => setReplacementReviewOpen(true)} /> : null}
  </section>;
}

function FloorCard({ floor, selected, busy, onSelect, onEdit, onArchive, onDelete, onTop, onUp, onDown, onBottom }: FloorCardProps) { return <article className={`woning-item ${selected ? "selected" : ""}`} role="listitem"><button className="woning-select" onClick={onSelect}><strong>{floor.name}</strong><span>{floor.activeRoomCount ?? 0} kamers</span></button><div className="woning-actions"><button disabled={busy} onClick={onEdit}>Naam wijzigen</button><button disabled={busy} onClick={onTop}>Bovenaan</button><button disabled={busy} onClick={onUp}>Omhoog</button><button disabled={busy} onClick={onDown}>Omlaag</button><button disabled={busy} onClick={onBottom}>Onderaan</button><button disabled={busy} onClick={onArchive}>Archiveren</button><button disabled={busy} onClick={onDelete}>Verwijderen</button></div></article>; }
function RoomCard({ room, config, busy, onClimate, onEdit, onMove, onArchive, onDelete, onTop, onUp, onDown, onBottom }: RoomCardProps) { const climateUnavailable = room.isArchived === true || room.isEnabled === false; return <article id={`room-${room.id}`} tabIndex={-1} className="woning-item woning-room" role="listitem"><div><h5>{room.name}</h5><p>{roomTypeLabels[room.roomType as RoomType] ?? "Anders"}{room.familyMember?.name ? ` · ${room.familyMember.name}` : ""}</p><p>{climateUnavailable ? "Herstel of activeer deze kamer om klimaatinstellingen te wijzigen" : climateStatus(config)}</p></div><div className="woning-actions"><button disabled={busy || climateUnavailable} onClick={onClimate}>{config?.isConfigured ? "Klimaat bewerken" : "Klimaat instellen"}</button><button disabled={busy} onClick={onEdit}>Bewerken</button><button disabled={busy} onClick={onMove}>Verplaatsen</button><button disabled={busy} onClick={onTop}>Bovenaan</button><button disabled={busy} onClick={onUp}>Omhoog</button><button disabled={busy} onClick={onDown}>Omlaag</button><button disabled={busy} onClick={onBottom}>Onderaan</button><button disabled={busy} onClick={onArchive}>Archiveren</button><button disabled={busy} onClick={onDelete}>Verwijderen</button></div></article>; }
function FloorForm({ floor, busy, onCancel, onSave }: FloorFormProps) { const [name, setName] = useState(floor?.name ?? ""); return <Dialog title={floor ? "Verdieping bewerken" : "Verdieping toevoegen"} onCancel={onCancel}><form onSubmit={(e) => { e.preventDefault(); if (name.trim()) void onSave(name.trim()); }}><label className="settings-file-field"><span>Naam verdieping</span><input autoFocus value={name} onChange={(e) => setName(e.target.value)} required /></label><Actions busy={busy} onCancel={onCancel} save={floor ? "Opslaan" : "Verdieping toevoegen"} /></form></Dialog>; }
function RoomForm({ room, members, busy, onCancel, onSave }: RoomFormProps) { const [name, setName] = useState(room?.name ?? ""); const [type, setType] = useState<RoomType>(room?.roomType ?? RoomType.Other); const [member, setMember] = useState(room?.familyMember?.id ?? ""); return <Dialog title={room ? "Kamer bewerken" : "Kamer toevoegen"} onCancel={onCancel}><form onSubmit={(e) => { e.preventDefault(); if (name.trim()) void onSave(name.trim(), type, member || undefined); }}><label className="settings-file-field"><span>Naam kamer</span><input autoFocus value={name} onChange={(e) => setName(e.target.value)} required /></label><label className="settings-file-field"><span>Kamertype</span><select value={type} onChange={(e) => setType(Number(e.target.value) as RoomType)}>{roomTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label><label className="settings-file-field"><span>Gezinslid (optioneel)</span><select value={member} onChange={(e) => setMember(e.target.value)}><option value="">Geen gezinslid</option>{members.map((m: FamilyMember) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label><Actions busy={busy} onCancel={onCancel} save={room ? "Kamer opslaan" : "Kamer toevoegen"} /></form></Dialog>; }
function MoveForm({ room, floors, busy, onCancel, onMove }: MoveFormProps) { const [floorId, setFloorId] = useState(floors[0]?.id ?? ""); return <Dialog title="Kamer verplaatsen" onCancel={onCancel}><form onSubmit={(e) => { e.preventDefault(); void onMove(floorId); }}><p>Verplaats {room.name} naar een andere verdieping. De kamer behoudt dezelfde identiteit.</p><label className="settings-file-field"><span>Nieuwe verdieping</span><select value={floorId} onChange={(e) => setFloorId(e.target.value)}>{floors.map((f) => <option key={f.id} value={f.id}>{f.name}</option>)}</select></label><Actions busy={busy} onCancel={onCancel} save="Verplaatsen" /></form></Dialog>; }
function ConfirmDialog({ dialog, busy, onCancel }: ConfirmDialogProps) { return <Dialog title={dialog.title} onCancel={onCancel}><p>{dialog.body}</p><Actions busy={busy} onCancel={onCancel} save={dialog.action} onSave={dialog.run} /></Dialog>; }
function Dialog({ title, onCancel, children }: DialogProps) { return <div className="settings-surface-backdrop" role="presentation"><section className="settings-surface-dialog woning-dialog" role="dialog" aria-modal="true" aria-label={title}><header className="settings-surface-header"><h3>{title}</h3><button onClick={onCancel}>Sluiten</button></header><div className="settings-surface-body">{children}</div></section></div>; }
function Actions({ busy, onCancel, save, onSave }: ActionsProps) { return <div className="settings-surface-actions"><button type="button" onClick={onCancel}>Annuleren</button><button disabled={busy} onClick={onSave} type={onSave ? "button" : "submit"}>{busy ? "Opslaan…" : save}</button></div>; }
function Archived({ title, description, children }: ArchivedProps) { return <section className="woning-archived" aria-label={title}><h5>{title}</h5>{description ? <p>{description}</p> : null}{children}</section>; }
