...
Calendar restore is local-only and full restore only. The backend validates the entire export document before destructive actions, rejects unsupported format/schema versions, rejects invalid household ownership, identifiers, source references, timing shapes, and timezones, replaces existing calendar event sources and EventSeries for the seeded household only after validation, and preserves supplied identifiers when restoring valid source and series records. Failed validation leaves existing calendar data unchanged. Automatic pre-restore export is a future safety requirement, not current automation. Selective import, merge import, conflict resolution, Google Drive upload, Google Calendar import, ICS, recurrence runtime behavior, EventException runtime behavior, notifications, reminders, authentication, and timezone UI remain out of scope.
