...
### Completed remediation slice — Production/demo bootstrap separation
Phase 2 Slice 2.3 resolves `BOOT-01` and `UX-01`: production bootstrap now contains only an incomplete household and structural application defaults, while household members, lists/items, motivation goals, and calendar events are explicit fixtures. The frontend no longer substitutes static family members while API state is loading. The migration removes only an exact untouched legacy graph; mixed, edited, or avatar-customized graphs are preserved and marked for later administrator review. The `Demo` environment explicitly loads the localized `visual-full` scenario, and legacy test data is isolated in a test-only fixture. Fresh and upgrade PostgreSQL regressions, the full backend/frontend suites, builds, model-drift check, and isolated browser suite pass.

### Product-integrity remediation execution tooling
The Phase 2 remediation program now has an optional guarded PowerShell orchestrator for bounded unattended runs. It keeps the existing one-slice-per-run boundary by launching a fresh `codex exec` process for each slice, advances only after structured completion, documentation, validation, and scope gates pass, and requires explicit per-slice commits before any multi-slice sequence. Each slice receives up to three attempts by default. After attempts one and two fail, the runner first executes a separately logged repair phase and only then launches the next fresh slice attempt. Known child-profile NSwag failures use an allowlisted parent repair that runs pinned NSwag twice and proves second-run idempotence; other failures receive a dedicated Codex repair process constrained to root-cause repair rather than slice completion. Every slice failure and repair retains its own evidence. Dirty one-slice recovery snapshots its initial files and ignores only unchanged baseline files during completion review; new or modified files remain gated. Blocked and invalid-completion statuses are normalized for recovery; no failed attempt advances or commits, and execution stops after the third failed slice attempt. Parent restore failures are passed into the child prompt for diagnosis. Git stderr is retained separately and judged by exit code, so harmless line-ending warnings cannot become changed paths or completion failures. The tooling does not change Phase 2 product scope, push commits beyond the explicit per-slice option, or bypass viewport-analysis, approval, validation, or scope requirements.

### Completed remediation slice — Honest asynchronous member mutations
Phase 2 Slice 2.2 resolves `MEMBER-02`: family-member add, profile/avatar update, and removal now await API completion, disable duplicate submission while pending, keep mutation dialogs open, and report rejected requests without applying optimistic shell state or false success. Failed drafts remain available for retry, including onboarding member creation; successful profile and avatar updates survive refresh through the isolated browser/API/PostgreSQL path. This slice does not implement atomic onboarding, bootstrap cleanup, member restore, central family administration, or the broader Phase 7 shared error framework.

### Completed remediation slice — Canonical family-member avatar contract
Phase 2 Slice 2.1 resolves `MEMBER-01`: current frontend family-member create/update requests now send only normalized `avatarSelection`, while the backend retains single-field `avatarV2Config` compatibility and its existing legacy persistence projection. Canonical create/update succeeds through the real endpoint and round-trips through persistence, and the browser avatar-refresh scenario is now a normal passing regression. No DTO, OpenAPI, generated-client, migration, database-column, layout, or later Phase 2 behavior changed.

### Completed remediation slice — Browser E2E smoke suite
Phase 0 Slice 0.3 adds a pinned Playwright project and one-command isolated browser runner. Five Chromium scenarios cover clean-install onboarding, family-member avatar persistence, Home local-day event placement, Tasks hit targets, and primary-page viewport overflow. The four known product defects are executable expected failures, while the viewport guard passes at 1440x900 and 1366x768. Each run uses a generated PostgreSQL database and loopback API/Vite ports, then removes its processes, database, and failure artifacts without touching the household database.

### Completed remediation slice — Real family-member API contract tests
Phase 0 Slice 0.2 adds four isolated real-endpoint contract tests for frontend-shaped family-member create/update requests. The tests demonstrate the current `MEMBER-01` contradiction without changing the avatar contract: dual `avatarSelection` plus `avatarV2Config` requests return HTTP 400 and leave persistence unchanged, while canonical selection-only requests return 200/201 and survive database plus GET round trips. Slice 0.2A corrected offset-sensitive date-only Agenda arithmetic and made the Home weather-hour assertion validate browser-local time, removing the common frontend-gate blocker. Focused contract tests pass 4/4, affected frontend tests pass 44/44, the frontend suite passes 310/310, the backend suite passes 579/579, and the frontend production build passes.

### Completed remediation slice — PostgreSQL database baseline and upgrade fixtures
Phase 0 Slice 0.1 of the product-integrity remediation plan adds guarded real-PostgreSQL clean/upgrade test databases, preserves representative household and user data across a one-migration-behind upgrade, records the current Home Assistant resume-strategy model/schema drift, and provides a one-command test runner plus safety documentation. Slice 0.1A also makes the Home Assistant credential test fixture cross-platform and restores pre-existing environment state. Focused Home Assistant validation passes 28/28, focused PostgreSQL validation passes 2/2 with generated databases fully cleaned up, and the repository-wide backend gate passes 575/575.
...
### Completed Slice — Avatar Contacts V1 Production Readiness
Avatar Contacts V1 Slice 10 finalizes the implementation with consistency, accessibility, localization, and regression validation polish. The slice removes implementation-facing copy from People/member People forms, localizes shared decorative picker groups and relationship group labels, adds shared KnownPerson dialog semantics and delete confirmation, confirms no duplicate renderer/form/picker/ranking/grouping implementations were introduced, validates OpenAPI/client stability and migration script generation, and keeps Shopping, Tasks, and Agenda decorative avatar behavior unchanged.

### Completed Slice — Avatar Contacts V1 Agenda Decorative Avatar Follow-up
Agenda Decorative Avatar Follow-up finalizes Slice 9 correctness by adding explicit backend coverage for nullable decorative pairs, invalid/deleted/cross-household/mismatched references, deletion clearing, recurrence projection, split-series propagation, skipped/modified/restored occurrence behavior, and read-only imported event behavior. Frontend tests cover imported/recurring rendering, manual clearing, and Agenda API mapping. Imported events are intentionally not decoratable through Agenda APIs in this version; migration script inspection confirmed nullable columns, enum string persistence, the composite index, and nullable-pair check constraint.

Avatar Contacts V1 Slice 9 makes Agenda the third production decorative-avatar consumer by adding optional nullable FamilyMember/KnownPerson decorative references to Agenda event series, reusing the shared validation, picker, suggestions, and badge infrastructure, projecting series decorative metadata through generated recurring occurrences, preserving existing recurrence and imported-event semantics, and clearing only decorative references on FamilyMember/KnownPerson deletion. Home, Motivation, authentication, permissions, AI, and automatic attachment remain deferred.
Avatar Contacts V1 Slice 7 adds deterministic smart suggestions to the Shopping decorative avatar picker by introducing reusable local ranking infrastructure, centralized relationship aliases, explicit score thresholds, lightweight typo/prefix matching, and a Suggested picker section that remains manual-selection-only. Tasks, Agenda, Home, Motivation, authentication, permissions, AI, LLMs, and automatic attachment remain deferred.
...
### Completed Slice — Woning Climate Floor Plans Slice 6 Room Overlay Editor
Woning Climate Floor Plans Slice 6 adds the frontend Settings-side polygon editor for trusted floor-plan overlays. Users can enter the editor from Woning Settings for Floors with an active safe derivative, draw/edit Room-linked polygons, place/reset manual label anchors, review textual validation guidance, save Draft or Valid geometry, and perform explicit backend lifecycle actions including trust, needs-review, archive, restore, and delete. The workspace is viewport-bounded with Room list, canvas, toolbar, and validation panel regions, and phone-sized editing is clearly blocked. Asset upload/replacement review, runtime `Klimaat in huis`, climate readings, Stories, heating controls, provider mapping UI, and Home Assistant setup remain deferred.
...
## Completed Slice — Woning Climate Floor Plans Slice 16 Climate Story Deep-Link Integration
Climate Stories now open the existing runtime `Klimaat in huis` workspace with a minimal safe context: target workspace plus optional Floor ID, Room ID, translated climate context code, and originating Story ID. The runtime applies valid Floor/Room selection after live read-model data loads, resolves moved Rooms from current Floor data where available, keeps room-list fallback fully selectable without Trusted overlays, and shows a compact dismissible Story-context panel that remains secondary to current climate facts. Refresh preserves valid selection and updates live provider/overlay/control state without rewriting the original Story text; resolved issues are described factually. Configuration-related climate Stories expose an explicit existing Settings escalation action, with no automatic redirect and no duplicated editor. This frontend-only slice intentionally adds no backend Story generation, recommendations, comfort scoring, heating automation, historical charts, provider setup changes, screenshots, binary assets, handwritten API workaround, generated-client edits, or unrelated product scope.
...
### Completed remediation design slice — Calendar-field contract

Phase 3 Slice 3.1 (`TIME-01`) records the approved calendar-field contract for subsequent calendar remediation: manual writes carry `YYYY-MM-DD` dates plus optional `HH:mm` times under the authoritative household zone, not JavaScript instants. The contract covers all-day, timed, multi-day, recurrence, exceptions, splits, imported instants, and deterministic DST validation/resolution; Slice 3.2 must add its specified failing regression matrix before changing endpoints. This design-only slice changes no runtime behavior, API/OpenAPI/generated client, persistence, UI, or viewport layout. The backend build and 588-test suite, frontend build, and 325-test frontend suite passed. Parent-owned pinned NSwag 14.7.1 generation also ran twice successfully with no OpenAPI or generated-client drift.
