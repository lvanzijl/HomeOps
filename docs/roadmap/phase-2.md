...
The Phase 2 remediation program now has an optional guarded PowerShell orchestrator for bounded unattended runs. It keeps the existing one-slice-per-run boundary by launching a fresh `codex exec` process for each slice, advances only after structured completion, documentation, validation, and scope gates pass, and requires explicit per-slice commits before any multi-slice sequence. Each slice receives up to three attempts by default: every failed attempt writes a retained failure report, and the next fresh Codex process must inspect that report, diagnose the root cause, preserve valid work, repair recoverable environment or implementation failures, and rerun the same slice. Blocked and invalid-completion statuses are normalized for recovery; the runner never advances or commits until a completion passes every gate, and it stops after the third failed attempt. Parent restore failures are passed into the child prompt for diagnosis instead of preventing recovery from starting. It does not change Phase 2 product scope, push commits beyond the explicit per-slice option, or bypass viewport-analysis, approval, validation, or scope requirements.
