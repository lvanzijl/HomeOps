# FamilyBoard Marketing Household

## Purpose

This document defines the canonical FamilyBoard marketing household for VisualReview marketing fixtures, marketing videos, product screenshots, documentation, website imagery, and future demonstrations.

This is a design artifact only. It must not be treated as production seed data, a VisualReview fixture, an API contract, a database schema, or a test specification.

## Canonical Household Identity

- **Household name:** Van Zijl family
- **Home city context:** Utrecht area, Netherlands
- **Primary language in UI examples:** Dutch for family-facing content, with product/navigation labels matching the current screen being demonstrated.
- **Canonical fixture date:** Tuesday, 16 June 2026
- **Canonical week:** Monday, 15 June 2026 through Sunday, 21 June 2026
- **Canonical month:** June 2026
- **Household tone:** Warm, practical, slightly busy, child-centered, and realistic rather than perfectly organized.

The family is intentionally ordinary: two working parents, one school-age child, one toddler in daycare, recurring sports, groceries, birthdays, small chores, and a calm Sunday Weekly Reset ritual.

## Family Members

| Member | Display name | Age | Role | Personality | Avatar concept |
| --- | --- | ---: | --- | --- | --- |
| Dad | Dad | 38 | Parent / planning anchor for sports, errands, and weekend logistics | Patient, dry humor, likes clear lists, tends to remember gear and transport but forgets small school notes | Warm medium skin tone, short dark-brown soft crop, light stubble, round glasses, navy hoodie, calm smile |
| Mom | Mom | 36 | Parent / emotional anchor for school, daycare, birthdays, and family rhythm | Energetic, caring, pragmatic, likes routines but accepts mess, notices when someone needs encouragement | Light skin tone, shoulder-length wavy auburn hair, soft green sweater, small hoop earrings, bright but tired smile |
| Thomas | Thomas | 6 | Older child / groep 3 pupil, learning responsibility with small tasks | Curious, chatty, football-obsessed, proud when trusted, sometimes slow in the morning | Light skin tone, sandy-blond tousled hair, freckles, blue football shirt, optional small star accessory |
| Robin | Robin | 2 | Younger child / daycare toddler, included through parent-managed routines | Cheerful, determined, loves animals and snacks, unpredictable at transitions, naps still matter | Light skin tone, soft brown curly toddler hair, yellow jumper, rounded cheeks, no complex accessories |

### Member Rules

- Dad is always Dad and always 38 years old.
- Mom is always Mom and always 36 years old.
- Thomas is always six years old.
- Robin is always two years old.
- Thomas is the only school-age child; Robin attends daycare.
- Avatar appearance must stay stable across screenshots and videos unless a future design document intentionally edits it.
- Child tasks must remain age-appropriate: Thomas can tidy, pack simple school items, and help set the table; Robin does not independently own operational tasks.

## Household Story

### Weekday Routine

Weekdays start around 06:45. Dad makes coffee and checks the FamilyBoard home screen while Mom gets breakfast started. Thomas needs reminders for shoes, school bag, and football days. Robin wants to choose a cup, then usually needs help getting dressed. School drop-off is at 08:20 and daycare drop-off follows at 08:35.

Afternoons are staggered. Mom usually handles school pickup on Tuesdays and Thursdays. Dad handles football practice on Wednesday and Saturday. Dinner is simple on activity days: pasta, wraps, soup, or leftovers. FamilyBoard is checked again after dinner for tomorrow's agenda and tasks.

### Weekend Routine

Saturday is for football, errands, playground time, and visiting family when energy allows. Sunday is calmer: laundry, meal planning, library books, and a short Weekly Reset after Robin is in bed. Thomas joins the first part of the reset to celebrate helpful moments and choose one small focus for the coming week.

### Family Values

- Everyone helps in a way that fits their age.
- Routines should reduce stress, not create a perfect household.
- Appreciation is more important than points or rewards.
- Planning is shared visibly so one parent is not the only memory keeper.
- The children should see the family working as a team.

### How FamilyBoard Fits Naturally

FamilyBoard is a visible family dashboard in the kitchen/living area. In the morning it answers: “What matters today?” In the evening it answers: “What do we need for tomorrow?” On Sunday it helps the family close the week gently: keep what worked, carry forward what still matters, and notice the small helpful moments.

## Home Content for Canonical Fixture Date

Use Tuesday, 16 June 2026 as the default Home screenshot day.

### Today's Agenda

| Time | Item | Member/context | Notes |
| --- | --- | --- | --- |
| 08:20 | Thomas naar school | Thomas, Mom | School bag and fruit snack needed. |
| 08:35 | Robin naar daycare | Robin, Mom | Spare clothes should be in the daycare bag. |
| 15:15 | Playdate: Thomas bij Noor | Thomas | Pickup at Noor's house at 17:00. |
| 17:30 | Snelle pasta voor zwemles | Household | Keep dinner easy before swimming. |
| 18:15 | Zwemles Thomas | Thomas, Dad | Bring towel, goggles, and clean clothes. |

### Today's Tasks

| Task | Owner | Status | Agenda link |
| --- | --- | --- | --- |
| Zwemtas klaarzetten | Dad | Open | Supports Thomas's swimming lesson. |
| Fruitbakje en drinkbeker school | Mom | Open | Supports school day. |
| Cadeau voor Noor inpakken | Thomas with Mom | Open | Supports playdate/birthday visit later this week. |
| Reservekleren daycare tas | Mom | Open | Supports Robin's daycare drop-off. |

### Shopping Summary

- **Albert Heijn:** volkoren pasta, passata, bananen, yoghurt, knijpfruit, koffiefilters.
- **Jumbo:** zwemluiers for Robin, wraps, mild grated cheese, cucumber, apple juice boxes.
- **Kruidvat:** toothpaste for children, sunscreen, plasters.
- **Bakker:** small birthday rolls for Saturday morning.

### Family Goal

- **Goal:** 20 helpful moments before Sunday pancake breakfast.
- **Progress on Tuesday morning:** 12 / 20 helpful moments.
- **Celebration:** Pancakes on Sunday morning if the goal is complete; the Sunday Weekly Reset records the celebration.

### Recent Appreciation

“Thomas helped Robin find her bunny before daycare, even though his own shoes were still missing.”

### Motivation Summary

- Helpful moments are coming from small morning and evening contributions, not from a chore economy.
- Thomas is close to his individual reading goal for the week.
- Parents are using appreciations to notice cooperation, patience, and remembering gear.

## Agenda: Recurring June 2026 Month

The month should feel naturally busy but not overloaded. Use recurring structure plus a few one-off events.

### Recurring Weekly Events

| Pattern | Time | Title | Member/context |
| --- | --- | --- | --- |
| Monday-Friday | 08:20-14:45 | School Thomas | Thomas |
| Monday, Tuesday, Thursday | 08:35-17:15 | Daycare Robin | Robin |
| Monday | 19:30-20:00 | Weekplanning ouders | Dad, Mom |
| Tuesday | 18:15-18:45 | Zwemles Thomas | Thomas, Dad |
| Wednesday | 16:30-17:30 | Football training Thomas | Thomas, Dad |
| Friday | 17:00-17:45 | Weekboodschappen | Mom or Dad |
| Saturday | 09:30-10:30 | Football match/training | Thomas, Dad |
| Sunday | 19:30-19:50 | Weekly Reset | Household; Thomas joins first 10 minutes |

### One-Off June Events

| Date | Time | Title | Member/context | Notes |
| --- | --- | --- | --- | --- |
| Tue 2 Jun | 15:30-16:30 | Dentist check-up | Thomas, Mom | Calm appointment after school. |
| Thu 4 Jun | 10:00-11:00 | Consultatiebureau | Robin, Dad | Toddler growth check. |
| Sat 6 Jun | 14:00-16:30 | Visit opa and oma | Household | Bring drawings from Thomas. |
| Sun 7 Jun | 19:30-19:50 | Weekly Reset | Household | Reset after family visit weekend. |
| Wed 10 Jun | 12:30-14:30 | School half day | Thomas | Extra pickup needed. |
| Fri 12 Jun | 18:00-20:00 | Pizza with neighbours | Household | Keep shopping light. |
| Sat 13 Jun | 10:00-11:00 | Football mini tournament | Thomas, Dad | Packed snacks needed. |
| Tue 16 Jun | 15:15-17:00 | Playdate at Noor's | Thomas | Canonical Home day. |
| Tue 16 Jun | 18:15-18:45 | Zwemles Thomas | Thomas, Dad | Canonical Home day. |
| Thu 18 Jun | 09:00-09:15 | Bring library books to school | Thomas, Mom | Supports school reading. |
| Sat 20 Jun | 11:00-13:00 | Noor's birthday party | Thomas | Gift and card needed. |
| Sun 21 Jun | 10:00-12:00 | Father's Day breakfast | Household | Pancake celebration if goal is complete. |
| Sun 21 Jun | 19:30-19:50 | Weekly Reset | Household | Close canonical week. |
| Wed 24 Jun | 16:30-17:30 | Last football training before break | Thomas | Bring team shirt. |
| Fri 26 Jun | 15:00-16:00 | Daycare summer picnic | Robin, Mom | Bring fruit and blanket. |
| Sun 28 Jun | 15:00-17:00 | Family visit: tante Lotte | Household | Coffee, fruit, and simple cake. |
| Sun 28 Jun | 19:30-19:50 | Weekly Reset | Household | Prepare final school week. |

## Tasks

Tasks should support the agenda directly and remain practical. Avoid inventing tasks that imply unsupported features.

### Today: Tuesday, 16 June 2026

| Task | Owner | Due | Motivation link |
| --- | --- | --- | --- |
| Zwemtas klaarzetten | Dad | Today | Counts as helpful household preparation. |
| Fruitbakje en drinkbeker school | Mom | Today | Morning routine support. |
| Cadeau voor Noor inpakken | Thomas with Mom | Today | Thomas contributes by choosing the card sticker. |
| Reservekleren daycare tas | Mom | Today | Prevents daycare stress. |
| Pasta saus uit vriezer halen | Dad | Today | Supports quick dinner before swimming. |

### Tomorrow: Wednesday, 17 June 2026

| Task | Owner | Due | Agenda link |
| --- | --- | --- | --- |
| Football shirt wassen | Dad | Tomorrow | Needed before Wednesday training/Saturday match. |
| Bibliotheekboeken zoeken | Thomas with Mom | Tomorrow | Needed for Thursday school return. |
| Daycare forms checken | Mom | Tomorrow | Keeps Robin's daycare notes current. |

### Later

| Task | Owner | Timing | Agenda link |
| --- | --- | --- | --- |
| Cadeauzakje en kaart voor Noor kopen | Mom | Before Sat 20 Jun | Birthday party. |
| Picknickkleed wassen | Dad | Before Fri 26 Jun | Daycare summer picnic. |
| Father's Day pancake ingredients checken | Mom and Thomas | Before Sun 21 Jun | Family goal celebration. |
| Library tote by front door | Thomas | Thu 18 Jun morning | School library return. |
| Plan simple dinner after football | Dad | Wed 24 Jun | Last training before break. |

### Recently Completed

| Task | Completed | Owner | Appreciation/agenda link |
| --- | --- | --- | --- |
| Football water bottle filled | Mon 15 Jun | Thomas with Dad | Thomas remembered after one reminder. |
| Weekend laundry folded | Sun 14 Jun | Dad and Mom | Kept Monday morning easier. |
| Robin's daycare sunscreen labelled | Sun 14 Jun | Mom | Supports warm June daycare days. |
| Grocery bags returned to hallway hook | Fri 12 Jun | Thomas | Helpful moment toward family goal. |

## Shopping

Shopping content should be grouped by real Dutch shopping destinations and should reflect children, meals, household basics, and upcoming events.

### Albert Heijn - Weekboodschappen

- Volkoren pasta
- Passata
- Cherry tomatoes
- Cucumbers
- Bananas
- Apples
- Greek yoghurt
- Muesli
- Wholegrain bread
- Peanut butter
- Mild cheese slices
- Coffee beans
- Oat milk

### Jumbo - Kids and Easy Dinners

- Mini wraps
- Mild grated cheese
- Chicken pieces or vegetarian pieces
- Sweetcorn
- Apple juice boxes
- Raisin boxes
- Knijpfruit
- Rice cakes
- Zwemluiers for Robin

### Kruidvat - Care and Household

- Children's toothpaste
- Kids sunscreen SPF 50
- Plasters
- Wet wipes
- Laundry stain spray
- Tissues for school bag

### Bakker - Weekend

- Croissants for Father's Day breakfast
- Small soft rolls for Saturday
- Simple apple cake for tante Lotte visit if using the 28 June fixture

### HEMA - Birthday/School Extras

- Birthday card for Noor
- Small sticker sheet for Thomas to add to the card
- Gift bag
- Extra hair clips for daycare bag

## Motivation

### Family Goal

- **Title:** 20 helpful moments before Sunday pancake breakfast
- **Target:** 20 helpful moments
- **Canonical progress:** 12 / 20 on Tuesday morning; 20 / 20 during Sunday Weekly Reset if showing the completed state.
- **Celebration:** Sunday pancake breakfast together.
- **Meaning:** The goal recognizes cooperation and preparation, not payment for chores.

### Individual Goals

| Member | Goal | Target | Progress on Tue 16 Jun | Visual tone |
| --- | --- | ---: | ---: | --- |
| Thomas | Read together 4 times this week | 4 reads | 2 reads | Stars/book path |
| Robin | Smooth daycare goodbyes | 3 calm mornings | 1 morning | Parent-observed gentle badges |

### Appreciations

- Dad appreciated Mom for packing the daycare bag before breakfast when Robin had a hard morning.
- Mom appreciated Dad for keeping swimming calm by putting the towel and goggles near the door.
- Dad and Mom appreciated Thomas for helping Robin find her bunny before daycare.
- Mom appreciated Thomas for putting the grocery bags back on the hallway hook.
- Dad appreciated Robin for trying to put her shoes by the mat, even though they were on the wrong feet.

### Celebrations

- Planned: Pancake breakfast on Sunday 21 June when the family reaches 20 helpful moments.
- Recent memory option for later fixtures: “Pizza with neighbours” on Friday 12 June can appear as a warm social memory, not as a reward purchase.

### Statistics

- 12 helpful moments logged this week by Tuesday morning.
- 4 recently completed household tasks visible in the week recap.
- 2 reading moments completed for Thomas.
- 1 calm daycare goodbye for Robin.
- 1 family celebration planned.

## Weekly Reset

Treat Weekly Reset as a calm Sunday evening ritual, usually 19:30-19:50 after Robin is asleep. Thomas joins the first 10 minutes for recognition, then Dad and Mom finish carried-forward planning.

### Completed Tasks for Canonical Week

- Zwemtas klaarzetten.
- Fruitbakje en drinkbeker school.
- Cadeau voor Noor inpakken.
- Football shirt washed.
- Library books returned to school.
- Noor's birthday gift and card prepared.
- Father's Day pancake ingredients checked.
- Grocery bags returned to hallway hook.

### Weekly Review

- The week was busy but manageable because swimming, playdate, birthday, and school-library tasks were visible early.
- Morning transitions were still messy, especially when Robin wanted to choose shoes herself.
- Thomas responded well to small concrete tasks and visible appreciation.
- Parents want fewer last-minute bag checks next week.

### Achievements

- Family goal reached: 20 / 20 helpful moments.
- Thomas completed 4 / 4 reading moments.
- Robin had 3 / 3 calmer daycare goodbyes with parent support.
- The family celebrated with pancakes on Sunday morning.

### Carried-Forward Items

- Picknickkleed wassen for daycare summer picnic on Friday 26 June.
- Check sunscreen supply again before the daycare picnic.
- Plan simple dinner after Wednesday football training.
- Keep library tote near the front door for school days.

### Focus for Next Week

“Prepare bags before bedtime on activity days.”

## Settings and Household Maintenance

Settings examples should communicate safety and confidence without exposing implementation details.

- **Latest backup:** Sunday, 21 June 2026 at 19:58, after Weekly Reset.
- **Calendar connection:** Family calendar connected and last refreshed Tuesday, 16 June 2026 at 07:05.
- **Restore availability:** Restore point available from Sunday, 21 June 2026 after Weekly Reset and from Friday, 19 June 2026 before weekend changes.
- **Household members:** Dad, Mom, Thomas, Robin.
- **Maintenance tone:** Reassuring and non-technical; avoid database, API, file, or infrastructure language.

## Consistency Rules for Future Marketing Fixtures

1. This is the canonical FamilyBoard marketing household.
2. Future marketing fixtures must derive household data from this document instead of inventing new names, ages, roles, avatars, or routines.
3. Dad is always Dad, age 38; Mom is always Mom, age 36; Thomas is always age 6; Robin is always age 2.
4. Thomas is always the school-age child; Robin is always the toddler in daycare.
5. Avatar concepts must remain visually stable unless a later design change explicitly updates this document.
6. Tasks must align with agenda events. For example, swimming tasks only appear near swimming lessons.
7. Shopping must align with meals, children, household needs, and upcoming events.
8. Appreciations must reference real completed tasks, realistic child behavior, or agenda-supported moments.
9. Motivation must remain encouragement-first. Do not turn helpful moments into money, points economy, store purchases, or sibling competition.
10. Weekly Reset must reflect the same week shown in Home, Agenda, Tasks, Shopping, and Motivation.
11. Home should show a compact summary of the same underlying story, not unrelated demo content.
12. The calendar may be busy, but fixtures should preserve breathable family realism and avoid every day feeling overloaded.
13. Dutch supermarkets should remain realistic: Albert Heijn, Jumbo, Kruidvat, HEMA, and a local baker are acceptable.
14. Settings content should stay family-facing and avoid technical implementation details.
15. Production code, APIs, database schema, fixtures, tests, screenshots, and binaries must not be changed to update this design document.

## Validation Review

The document has been reviewed for internal consistency:

- The canonical Home date, week, and month are aligned.
- Today's tasks support today's agenda.
- Shopping items support meals, children, household care, swimming, birthdays, and the Father's Day/pancake story.
- Motivation appreciations reference realistic completed or in-progress household moments.
- Weekly Reset closes the same canonical week that appears in Agenda, Tasks, and Motivation.
- The tone is family-friendly and suitable for screenshots, videos, documentation, and marketing imagery.
- The content remains a design document only and does not define production implementation details.
