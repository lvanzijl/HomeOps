...
- 2026-07-27: Hardened the remediation orchestrator after the Slice 2.4 recovery: structured outcomes now accept either the requested slice ID or the already-observed `ID - title` label while the prompt explicitly requires ID-only output, and child Codex processes inherit an available Node runtime plus the bundled package-manager fallback so frontend validation does not incorrectly report Node as unavailable.
