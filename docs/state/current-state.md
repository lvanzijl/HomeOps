- 2026-08-08: Phase 4 Slice 4.1 (`TASK-UI-01`, `TASK-UI-02`) is completed. Tasks removed generic click-to-select list items and selection-dependent action visibility; every card now exposes named title/details, Complete/Reopen, eligible Tomorrow, and More buttons directly. More renders through a viewport-clamped document portal with expanded/control state, first-item focus after positioning, Escape focus return, and outside-click/scroll closure, while the approved command/main-grid/secondary-rail composition remains unchanged. Focused Tasks tests pass 12/12, frontend 342/342, backend 622/622, both builds, and all six PostgreSQL-backed Playwright scenarios pass, including minimum 40×40 real hit targets, mouse/keyboard behavior, popup bounds, and no document overflow at 1280×720 plus established checks at 1440×900 and 1366×768. See `docs/reports/2026-08-08-task-actions/implementation.md`.
- 2026-08-08: Phase 4 Slice 4.0 is completed and Phase 4 is now in progress. The approved Tasks interaction authority preserves the current fixed command band, bounded Today/Planning grid, and horizontal secondary rail; the page itself already fits established viewports, while the known defect is selection-only card actions plus a More popup clipped by its overflow-hidden action rail. Slices 4.1–4.3 must use a non-interactive list item, named title/details control, always-visible Complete/Reopen and eligible Tomorrow actions, a viewport-clamped portalled More menu with keyboard/focus behavior, and bounded archive/routine surfaces. This documentation-only slice made no runtime, API, schema, CSS, test, generated-client, or binary change. See `docs/reports/2026-08-08-tasks-interaction-analysis/viewport-analysis.md` and `docs/roadmap/phase-4.md`.
- 2026-08-08: Phase 3 Slice 3.7 (`CAL-01`) and Phase 3 are completed. HomeOps stores and displays appointments but does not send reminders or notifications; Agenda states this in its existing bounded details step and exposes no reminder controls, while Settings now labels operation feedback as in-app activity/status. Decision `docs/decisions/0007-reliable-reminders-deferred.md` requires persisted rules, background scheduling, acknowledgements, per-device permission/subscription lifecycle, household-zone behavior, and failure visibility before any future reminder phase. No reminder fields, permissions, timers, service workers, schedulers, delivery tables, or notification subscriptions were added. Backend 622/622, frontend 340/340, both builds, and Playwright 6/6 with the final details state at 1440x900 and 1366x768 pass; every Phase 3 exit criterion is checked.
- 2026-08-07: Phase 3 Slice 3.6 (`DEVICE-01`) is completed. Agenda layer visibility now uses schema-versioned JSON under `homeops.deviceIdentity.v1`; the legacy string key migrates in place, and settings APIs require device ID plus version while retaining no authentication significance. `DeviceSettingsIdentities` owns schema version, creation, and last-seen timestamps; migration backfills existing preference owners, child settings cascade, reads/writes touch activity, and a daily service removes identities inactive for more than 180 days. Agenda provides a confirmed bounded reset that deletes the old server preferences, creates a fresh local identity, and reloads defaults under `docs/reports/2026-08-07-device-settings-identity/viewport-analysis.md`. Backend 622/622, frontend 339/339, PostgreSQL 3/3 through Rancher Desktop, Playwright 6/6 at both target viewports, both builds, EF drift/script checks, and twice-identical pinned NSwag 14.7.1 generation pass.
- 2026-08-07: Phase 3 Slice 3.5 (`CAL-02`, `CAL-03`) is completed. Real multipart `.ics` upload/replacement validates UTF-8, the 5 MiB limit, parse results, and duplicate UIDs before persistence; opaque managed storage records server-computed SHA-256, filename, size, and upload time while public source contracts expose no storage reference request or response. HTTPS feed reconnect preflights proposed content, archive hides events while retaining configuration/content, restore refreshes before exposure, and confirmed removal deletes imported series, configuration, source, and managed file. Settings implements the lifecycle in bounded dialogs under `docs/reports/2026-08-07-calendar-source-lifecycle/viewport-analysis.md`. Backend 615/615, frontend 336/336, PostgreSQL 3/3 through Rancher Desktop, Playwright 6/6 at both target viewports, builds, EF drift/script checks, and twice-identical pinned NSwag 14.7.1 generation pass.
- 2026-08-07: Phase 3 Slice 3.4 (`SETTINGS-02`) is completed. Settings adds a bounded searchable household IANA-zone dialog with server impact preview, explicit confirmation, pending state, and retained source-specific failures. The update force-loads every enabled iCal source without conditional cache shortcuts before entering one transaction; failures leave zone/events untouched, while success changes the zone, synchronizes prepared imports, and marks disabled imports stale until a successful refresh permits re-enabling. Manual calendar fields remain unchanged and read projections use the new zone. Migration `20260807202026_AddEventSourceNormalizationTimeZone` records source normalization state. The approved viewport report is `docs/reports/2026-08-07-household-time-zone/viewport-analysis.md`. Backend 606/606, frontend 334/334, required PostgreSQL 6/6, client/backend builds, six Playwright scenarios at both target viewports, EF drift/script checks, and twice-identical pinned NSwag 14.7.1 generation pass.
- 2026-08-07: Phase 3 Slice 3.2 (`TIME-01`) is completed. Manual event writes now accept household-local calendar fields, occurrence modification/splitting use an atomic timing group, and one resolver enforces range and DST rules while projecting reads in the authoritative household IANA zone. iCalendar UTC/`TZID` values normalize to that zone while floating/all-day values retain calendar semantics. Existing writable manual events are versioned as repair candidates; the API supports read-only listing, preview, explicit confirmation, and optimistic-concurrency repair without automatic shifts. A deprecated UTC-input compatibility shim remains until Slice 3.3 migrates the frontend. Builds, 598 backend tests, 325 frontend tests with the documented extended timeout, migration/model-drift checks, and twice-idempotent pinned NSwag 14.7.1 generation pass.
- 2026-08-02: Corrected dirty one-slice recovery accounting in the remediation orchestrator. It now fingerprints the initial dirty worktree and excludes only unchanged baseline files from the final slice changed-file comparison, so a preserved pre-existing runner edit cannot invalidate an otherwise complete recovery. New, deleted, or modified baseline files remain gated. Completed-result prompts now prohibit failed diagnostic validations, and parent NSwag repair discards pnpm stdout before returning its structured repair object. This follows Slice 3.1 evidence: attempt two passed backend 588/588, frontend 325/325, build, and parent NSwag idempotence but was rejected only because a pre-existing runner edit was counted as unreported; attempt three was rejected only because its completed result included a deliberately failed diagnostic assertion.
- 2026-07-27: Corrected remediation recovery sequencing to run a distinct repair phase between failed slice attempts. Known NSwag child-profile failures now trigger an allowlisted parent action that runs pinned NSwag twice and verifies generated-client/OpenAPI idempotence; other failures launch a separately schema-constrained Codex repair process before the next full slice attempt. Git stdout/stderr are captured separately and exit code is authoritative, preventing LF/CRLF warnings from being treated as changed files or failed gates. Parser/dry-run validation, an isolated three-attempt/two-repair forced-failure simulation, the real parent NSwag repair with unchanged hashes, and Git warning separation all pass. The Slice 3.1 reports proved its second attempt had actually passed every slice validation and was rejected only by the former Git-warning bug.
- 2026-07-27: Hardened the remediation orchestrator with bounded per-slice recovery. Each slice now receives three attempts by default; every failure writes an ignored JSON report with prompt/result/event/progress evidence, and each fresh retry is instructed to diagnose and repair the previous root cause while preserving valid work. Blocked or invalid-completion plan statuses are reset to In progress before retry, parent restore failures are supplied to Codex for diagnosis, no failed attempt advances or commits, and the runner stops after the third failure. PowerShell parsing, dry-run selection, and an isolated forced-failure simulation verified exactly three attempts, three reports, two recovery prompts, and terminal stop behavior. See `docs/reports/2026-07-27-remediation-retry-recovery/implementation.md`.
- 2026-07-27: Completed Phase 2 Slice 2.6 (`ONB-03`) and Phase 2 of the product-integrity remediation program. Onboarding completion now shows a bounded optional setup checklist whose dismissal persists per household; list, non-system calendar-source, and active Home Assistant states are derived from durable data, while weather remains explicitly deferred to `WEATHER-01`. Existing completed households are migrated as dismissed. NSwag 14.7.1 is generated idempotently; backend passes 588/588, frontend 325/325, both builds and EF model-drift validation pass, PostgreSQL migration tests pass 3/3, and all 6 browser outcomes pass with dismissal surviving refresh and no document overflow at 1366×768. See `docs/reports/2026-07-27-first-run-setup-checklist/implementation.md`.
- 2026-07-27: Completed Phase 2 Slice 2.5 (`FAMILY-01`, `FAMILY-02`, `SETTINGS-04`). A bounded Settings `Gezinsleden` surface now supports empty-roster add, shared-form add/edit, dependency-aware soft removal, removed-member listing, and guarded restore without reassigning task, room, goal, or private-known-person references. Completed households with zero active members stay out of onboarding. NSwag 14.7.1 output is generated and idempotent; focused backend passes 14/14, focused frontend 19/19, backend 587/587, frontend 323/323, restore/build/model-drift gates pass, and all 6 browser outcomes pass with add/remove/restore persistence and 1440x900 plus 1366x768 viewport coverage.
- 2026-07-27: Hardened the remediation orchestrator after the Slice 2.4 and 2.5 recoveries: structured outcomes accept either the requested slice ID or the observed `ID - title` label while the prompt explicitly requires ID-only output; child Codex processes inherit an available Node runtime plus the bundled package-manager fallback; NSwag is pinned to 14.7.1; and the parent now runs the required .NET restore before each slice so the child sandbox does not probe the inaccessible user NuGet configuration.
- 2026-07-27: Completed Phase 2 Slice 2.4 (`ONB-01`, `ONB-02`). Onboarding now keeps household/member drafts local until a single validated request writes the reviewed collection and completion state transactionally; review supports removal and correction, completed retries do not duplicate members, and status-load failure blocks app entry with retry. NSwag 14.7.1 regenerated OpenAPI/client output idempotently. Focused backend passes 3/3, focused frontend 18/18, backend 584/584, frontend 319/319, both builds and EF model-drift validation pass, and all 5 browser outcomes pass with atomic completion surviving refresh.
- 2026-07-27: Completed Phase 2 Slice 2.3 (`BOOT-01`, `UX-01`). Production databases now bootstrap an incomplete household plus structural event-source, workspace-layout, and task-template defaults without demo members, lists/items, motivation goals, or events; the frontend no longer renders static fallback members. Demo mode explicitly applies the localized `visual-full` fixture, while historical test data is isolated in a test-only fixture. Migration `20260727121951_SeparateProductionBootstrapFromDemoData` removes only an exact untouched legacy graph and preserves/marks mixed or avatar-edited graphs for administrator review. PostgreSQL migration tests pass 3/3, backend 583/583, frontend 318/318, both builds and EF model-drift validation pass, and all 5 browser outcomes pass with only the deferred local-day and task-control scenarios remaining expected failures.
- 2026-07-27: Added a guarded PowerShell orchestrator for the product-integrity remediation plan. It selects bounded slices, runs one non-interactive Codex process per slice, validates JSONL lifecycle events plus a schema-constrained result, requires plan/state/roadmap/report and validation evidence before advancing, stops on blocked or failed work, stores ignored local run evidence, and requires explicit per-slice commits for multi-slice execution. The tooling does not execute a remediation slice, push commits, open pull requests, or weaken the repository's viewport, scope, cache, or validation rules.
- 2026-07-27: Completed Phase 2 Slice 2.2 (`MEMBER-02`). Family-member add, profile/avatar update, and removal now await API completion; pending actions disable duplicate submission and keep dialogs open, failures retain drafts with visible retry feedback, and rejected profile saves no longer alter shell state or show success. Onboarding member creation also retains fields after failure. Focused mutation tests pass 39/39, full frontend passes 317/317, full backend passes 580/580, both builds pass, and the isolated browser suite passes all 5 outcomes while proving avatar and profile updates survive refresh; three unrelated known defects remain expected failures.
- 2026-07-27: Completed Phase 2 Slice 2.1 (`MEMBER-01`). Family-member create/update adapters now serialize only canonical `avatarSelection`, including normalization from legacy-only in-memory members. The backend still accepts a single legacy `avatarV2Config`, rejects mixed representations, and maintains its existing legacy storage projection; DTOs, OpenAPI, generated client, database schema, and UI layout are unchanged. Focused frontend tests pass 2/2, real-endpoint contract tests pass 5/5, the isolated browser avatar save/refresh scenario is now a normal pass, full frontend passes 310/310, full backend passes 580/580, and both builds pass.
- 2026-07-27: Completed Phase 0 Slice 0.3 and Phase 0 of the product-integrity remediation program. Added pinned Playwright 1.62.0 browser coverage and a root `pnpm test:e2e` runner that creates a guarded disposable PostgreSQL database, starts E2E API/Vite services on loopback ports, waits for health, and cleans up the generated database and processes. Five Chromium scenarios now cover onboarding, avatar persistence, Home local-day events, Tasks hit targets, and primary-page viewport overflow. The suite initially recorded four expected defect scenarios; Slice 2.1 has since promoted avatar persistence to a normal pass, while the viewport guard remains passing at 1440x900 and 1366x768. A real-browser check independently confirmed zero Tasks page overflow and obscured “Klaar”/“Morgen” targets. No API contract, migration, generated client, or primary-page layout was changed.
- 2026-07-27: Completed Phase 0 Slices 0.2 and 0.2A. Four fresh-host tests now assert HTTP status, validation details, unchanged persistence for rejected dual-avatar family-member requests, and database plus GET round trips for canonical selection-only requests. Corrected offset-sensitive date-only Agenda addition by using UTC calendar fields and made the Home weather-hour assertion timezone-independent while preserving browser-local display behavior. Focused contract tests pass 4/4, affected frontend tests pass 44/44, the frontend suite passes 310/310, the backend suite passes 579/579, and the frontend production build passes. No API contract, migration, generated client, layout, or avatar product behavior changed.
- 2026-07-27: Completed Phase 0 Slices 0.1 and 0.1A for the product-integrity remediation program. Added guarded `homeops_test_<guid>` database creation/cleanup, clean and one-migration-behind migration tests, representative household/user-row preservation checks, a one-command PowerShell runner, and database-testing safety documentation. Corrected the Home Assistant credential test fixture to handle case-insensitive Windows environment-variable aliases and restore any pre-existing value. Focused Home Assistant tests pass 28/28, focused real-PostgreSQL tests pass 2/2 with no generated databases left behind, and the full backend suite passes 575/575. No frontend, API contract, migration, generated client, or product behavior changed.
- 2026-07-23: Made the FamilyBoard frontend installable as an Android PWA by adding a web app manifest with standalone display, root start URL/scope, theme/background metadata, an SVG app icon with maskable purpose, static app-shell service worker registration, and install metadata in the Vite HTML shell. HTTPS remains a deployment requirement for production origins and is satisfied only by serving the built app over HTTPS or localhost during development. No primary page layout, backend, API, database, authentication, or feature-domain behavior was changed.
- 2026-07-17: Completed Woning Climate Floor Plans Slice 17 final audit hardening. The audit verified the accepted configuration-to-runtime climate/floor-plan flow, regenerated NSwag with no contract/client drift, ran focused backend and frontend climate/floor-plan validation, fixed concrete production frontend `any` leftovers in Woning Settings and the Room Overlay Editor with explicit local types, documented the release audit, and left product behavior unchanged. The deferred full release-gate matrix now passes locally: backend full suite twice, frontend full suite three consecutive normal runs plus one single-worker run, builds/type-check/NSwag/migration/diff checks, and the final release recommendation is Ready to merge/release.
- 2026-07-17: Implemented Woning Climate Floor Plans Slice 16 climate Story deep-link integration in the frontend. Climate Stories now carry a minimal safe runtime context to the existing `Klimaat in huis` workspace, apply valid Floor/Room selection after live climate data loads, resolve moved Rooms from live Floor state where possible, show a compact dismissible Story-context panel, keep current read-model climate values authoritative, preserve selected Rooms on refresh, support room-list fallback without Trusted overlays, and expose explicit Settings escalation for configuration Stories. No backend Story generation, new recommendations, comfort scoring, automation, historical charts, screenshots, binary assets, handwritten API workaround, generated-client edit, or unrelated product scope was added.
- 2026-07-17: Implemented Woning Climate Floor Plans Slice 12 runtime room heating controls in the selected-Room detail of `Klimaat in huis`, using generated heating-control contracts directly for capability, supported actions, blockers, ranges, durations, active override, latest command, and generated command submissions. The UI separates observed climate facts from requested/confirmed temporary override state, supports temporary warmer, generated-supported temporary cooler, and schedule resume with idempotent ambiguous retry UX, Dutch command-state presentation, shared-zone warning, and focused frontend tests. No handwritten API workaround, generated-client edit, optimistic climate mutation, schedule editor, real provider integration, Stories, weather/presence automation, Home Assistant setup, screenshots, binary assets, or unrelated product scope was added.
...
- Agenda layer settings persist through backend APIs per schema-versioned browser/device identity, with independent Week and Months selections.
...
- The frontend stores a versioned local preference identity and sends `X-HomeOps-Device-Key` plus `X-HomeOps-Device-Version`; the identity is not an account, credential, or authorization mechanism.
...
- 2026-08-02: Phase 3 Slice 3.1 (`TIME-01`) is completed. Its calendar-field contract requires manual calendar writes to express `DateOnly` plus optional `TimeOnly` under the authoritative household time zone, rather than sending a JavaScript instant for reinterpretation. It defines all-day/timed/multi-day/recurrence/exception/split invariants, rejects nonexistent local DST times, deterministically uses the first occurrence for ambiguous fall-back times, and keeps imported UTC/provider timestamps on a separate normalization boundary. Build and full backend/frontend test gates passed (588 backend and 325 frontend tests). The parent repair ran pinned NSwag 14.7.1 twice successfully and confirmed no OpenAPI or generated-client drift. No runtime, OpenAPI, generated-client, persistence, or UI behavior changed.
- 2026-08-07: Phase 3 Slice 3.3 (`TIME-01`, `TIME-02`, `HOME-01`) is completed. Home and Agenda manual writes share one literal calendar-field mapper for create, series edit, occurrence edit, split, and quick-add; no manual write creates a JavaScript instant or calls `toISOString()`. Household-zone state comes from the onboarding/current-household response, projected reads are restored to form fields in that zone, and Home resolves today/tomorrow at action time. `Meer opties` transfers only the current all-day draft into Agenda. Settings adds bounded, one-at-a-time `Kalendercontrole` preview/confirm/repair with retained conflict and validation input. The deprecated UTC request shim is removed from runtime DTOs, OpenAPI, and the generated client. The approved viewport report is `docs/reports/2026-08-07-calendar-fields-frontend/viewport-analysis.md`. Builds, 598 backend tests, 332 frontend tests with the documented extended timeout, six Playwright scenarios including TIME-01 as a normal pass, target viewport checks, and twice-idempotent pinned NSwag 14.7.1 generation pass.
