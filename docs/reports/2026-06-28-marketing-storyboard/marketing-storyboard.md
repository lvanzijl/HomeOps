...
Polished the canonical FamilyBoard marketing preview storyboard at `docs/design/marketing-storyboard-v1.md` to strengthen continuity, branded Chapter Card usage, shopping continuity, pacing, and emotional storytelling before the first preview recording.
...
6. Practical shopping where existing `Bloem`, `Roomboter`, `Chocoladestukjes`, and `Vanillesuiker` support `Koekjes bakken`, with one quick `Bananen` addition.
...
This curve is intended to guide future Marketing Director pacing, transitions, branded Chapter Cards, touch restraint, and audio decisions. The polished storyboard also ties Home, Family, Agenda, Tasks, Shopping, Motivation, Weekly Reset, and Outro into the same ordinary week.
...
| Shopping | `visual-marketing-shopping` | Shopping | 7s | Keep baking ingredients present for `Koekjes bakken` and add only `Bananen`. |
...
## Chapter Cards and visual language

The storyboard now treats Chapter Cards as a FamilyBoard brand element rather than a persistent feature overlay. Each scene begins with a brief branded card containing the title and optional subtitle, holds for approximately one second, fades out gently, and leaves the interface to own the full screen.

The added visual language guidance emphasizes calm transitions, touch-first restraint, breathing room between actions, subtle branding, and story before features.
...
- No persistent feature overlays: Yes; Chapter Cards fade out before each scene continues.
- Every interaction has purpose: Yes.
- Stronger continuity between scenes: Yes; swimming, Thomas, Filmavond, cookies, baking ingredients, bananas, appreciation, and Weekly Reset now carry through one family week.
...
- Home and Outro visually mirror one another: Yes; the Outro returns to Home, holds the dashboard, fades to a simple FamilyBoard brand card for approximately two seconds, then fades to black.
...
- **Is it compatible with the Marketing Director?** Yes. Scenes retain fixture, chapter copy, purpose, emotional tone, visual focus, min/preferred/max durations, Chapter Card direction, transition, and semantic action direction.
