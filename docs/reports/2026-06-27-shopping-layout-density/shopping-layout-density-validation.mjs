...
async function validateViewport(browser, viewport) {
  ...
  const densityMetrics = await page.evaluate(() => {
    ...
    return {
      ...
      lifecycleHeight: document.querySelector('.shopping-lifecycle-workspace')?.getBoundingClientRect().height ?? null,
      heroRect: rectFor('.shopping-workspace-hero'),
      quickAddRect: rectFor('.shopping-quick-add-panel'),
      storeWorkspaceRect: rectFor('.shopping-active-store-workspace'),
  ...
  await page.waitForFunction(() => [...document.querySelectorAll('.shopping-lifecycle-details')].some((section) => section.textContent?.includes('Afgevinkt') && section.textContent?.includes('Brood')));
  await page.locator('.shopping-lifecycle-details').filter({ hasText: 'Afgevinkt' }).locator(':scope > summary').click();
  const albertCountAfterToggle = (await albertCard.locator('.shopping-store-card-header span').innerText()).trim();
  await page.locator('.shopping-lifecycle-details').filter({ hasText: 'Afgevinkt' }).locator('.shopping-item').filter({ hasText: 'Brood' }).getByRole('button', { name: 'Terugzetten' }).click();
  ...
  await page.waitForFunction(() => [...document.querySelectorAll('.shopping-lifecycle-details')].some((section) => section.textContent?.includes('Recent verwijderd') && section.textContent?.includes('Zeep')));
  await page.locator('.shopping-lifecycle-details').filter({ hasText: 'Recent verwijderd' }).locator(':scope > summary').click();
  ...
  await page.locator('.shopping-lifecycle-details').filter({ hasText: 'Recent verwijderd' }).locator('.shopping-item').filter({ hasText: 'Zeep' }).getByRole('button', { name: 'Terugzetten' }).click();
  ...
  const result = {
    ...
    densityMetrics,
    postInteractionMetrics: metrics,
  };
