...
Avatar Contacts V1 establishes a **People** catalog for non-family-member identities. Each person has a name, optional nickname, relationship type, optional custom relationship label, initials, and avatar selection. A person exists so the household can recognize a person visually. It does not imply login access, household membership, task responsibility, permissions, calendar attendance, shopping assignment, or motivation ownership.
...
- People and family members may be combined only in read models for visual selection and management views.
...
`KnownPerson` does not require its own `DisplayColor`. Repository evidence shows `DisplayColor` is currently a `FamilyMember` field, but the reusable avatar renderer consumes `AvatarSelection`, and initials fallback can be generated from the person's display name. Avatar Contacts V1 does not need color parity with `FamilyMember` because People are lightweight visual identities, not semantic household members, and no current renderer requirement depends on a KnownPerson-specific color column. If a future implementation proves a color token is required for a specific shared fallback component, that should be justified in that implementation slice rather than added preemptively here.
...
Future picker APIs may project family members and `KnownPerson` records into a shared read DTO. That DTO should not become a shared domain base class. It should be a query shape only.
...
Use a **People** management capability with sections for family members and People. The eventual frontend UX may be a page, dialog, drawer, or extension of an existing surface; this V1 design does not require People to become a new primary page.
...
- It keeps shared People central while still preserving member-specific contexts.
...
Useful for member-specific relationships, but incomplete for Shared People. Member pages should expose quick editing for that member's friends and other private relationships, not become the whole People directory.

#### Hybrid People + member shortcuts

Recommended as the product principle. Shared People can be managed centrally. Private People primarily belong with the owning member and may also be reachable from a central People capability.
...
The UX should still make private People feel naturally owned by their member context, especially for children managing their own friends, classmates, and teachers from their own member page. Future authentication can introduce restrictions without changing the `KnownPerson` model.
...
Shared People belong in the central People capability. They should be grouped by presentation group rather than presented as one flat list. No filtering is required for V1; simple visual grouping is enough.

The groups shown inside Shared People are presentation groups only. They must not become domain concepts, persistence fields, API enums, or authorization boundaries. `KnownPersonRelationshipType` remains the canonical model, and the UI derives presentation groups from relationship types.
...
```text
Grandparent, Uncle, Aunt, Cousin -> Family
Teacher, Coach -> School
Babysitter, Neighbour -> Helpers
...
Decorative avatar persistence is intentionally out of scope for V1. The chosen `KnownPerson` architecture naturally supports future decorative avatar references because it provides stable household-scoped identity records without allowing `KnownPerson` records into semantic family-member fields.
...
- The source distinction keeps a decorative KnownPerson reference different from task ownership.
- Shopping items, tasks, and agenda events can each add optional decorative fields later without changing KnownPerson fundamentals.
...
A future picker endpoint may be added after KnownPerson CRUD exists:
...
- FamilyMemberId varchar(120) null foreign key FamilyMembers(Id) on delete restrict
...
IX_KnownPeople_HouseholdId_FamilyMemberId_IsDeleted_DisplayName
...
```text
(Scope = 'Shared' AND FamilyMemberId IS NULL)
OR
(Scope = 'PrivateToMember' AND FamilyMemberId IS NOT NULL)
```
...
Do not allow task ownership to point to either `FamilyMember` or `KnownPerson`. `KnownPerson` records cannot own tasks, and decorative avatar badges must remain visually separate from responsibility.
...
### Rejected: People details/address book
