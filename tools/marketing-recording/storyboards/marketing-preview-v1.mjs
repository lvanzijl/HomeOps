...
const agendaRecordingActions = Object.freeze({
  async validateDefaultPlanning({ page }) {
    ...
    await expectSingle(root.locator('.agenda-command-main').getByRole('button', { name: 'Gebeurtenis toevoegen', exact: true }), 'Agenda header Add event button');
    await holdReadable(page);
  },
  async showMonth({ page, touch }, action) {
    await ensureAgendaPage(page);
    await tapLocator(touch, agendaRoot(page).getByRole('button', { name: 'Maand bekijken', exact: true }), 'Agenda contextual Month action', action);
...
export const marketingPreviewV1Storyboard = Object.freeze({
  ...
  chapters: Object.freeze([
    ...
    { id: 'agenda', narrativeStep: 'planning', scenes: Object.freeze([scene({ id: 'agenda', fixture: 'visual-marketing-agenda', title: 'Agenda', subtitle: 'What the family needs to know next', purpose: 'Show the Planning briefing first, briefly open the contextual Month planner, then add one realistic plan from the header action.', narrativeRole: "Today's planning grows into a wider family rhythm with Filmavond.", emotionalTone: 'Confidence without rigidity', visualFocus: 'Planning briefing, contextual Month planner, header add-event action, saved Filmavond event, and returned planning overview.', minimumDurationMs: 11000, preferredDurationMs: 14000, maximumDurationMs: 15000, transition: { type: 'dissolve' }, cameraPacing: 'Hold on the Planning briefing first. Open Month briefly to confirm date-finding context. Keep event creation steady and readable, then return to Planning.', touchGestures: ['Tap Maand bekijken', 'Tap header add event', 'Enter Filmavond', 'Save', 'Tap Terug naar planning'], interactionSequence: ['Show default Planning briefing', 'Open Month and pause briefly', 'Use the header add-event action for Filmavond', 'Save', 'Hold briefly so Filmavond is visible', 'Return to Planning'], audioEvents: ['ChapterStarted', 'TouchStarted', 'TouchCompleted', 'ActionCompleted', 'save'], expectedFinalState: 'Agenda planning is visible with Filmavond added as a believable event.', directorNotes: 'The Planning briefing should feel like the default answer; Month is only a quick planning aid.', timing: { initialHoldMs: 1300, viewHoldMs: 900, postSaveHoldMs: 1200, postSceneHoldMs: 700 }, actions: [agendaAction('validate-default-planning', 'Validate and pause on the default Planning briefing.', agendaRecordingActions.validateDefaultPlanning, { resultHoldMs: 1000 }), agendaAction('show-month', 'Open the contextual Month planner.', agendaRecordingActions.showMonth, { hesitationMs: 80, holdAfterMs: 250, durationMs: 220, directClick: true, clickTimeoutMs: 5000 }), action('pause-month', 'pause', 'Pause on Month view.', { holdMs: 1000 }), agendaAction('add-filmavond', 'Add one event named Filmavond.', agendaRecordingActions.addFilmavond, { hesitationMs: 80, holdAfterMs: 180, durationMs: 220, directClick: true, clickTimeoutMs: 5000, typingDelayMs: 0 }), agendaAction('save-filmavond', 'Save Filmavond.', agendaRecordingActions.saveFilmavond, { hesitationMs: 80, holdAfterMs: 300, durationMs: 220, locatorClickSteps: 1 }), action('hold-filmavond-visible', 'pause', 'Hold briefly so Filmavond is visible.', { holdMs: 1000 }), agendaAction('return-agenda-planning', 'Return to the Planning overview.', agendaRecordingActions.returnOverview, { hesitationMs: 80, holdAfterMs: 250, durationMs: 220, directClick: true, clickTimeoutMs: 5000 })] })]) },
