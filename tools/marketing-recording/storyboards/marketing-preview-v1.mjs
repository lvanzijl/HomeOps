...
const taskRecordingActions = Object.freeze({
  async openAddDialog({ page, touch, eventBus, scene }, action) {
    await expectSingle(tasksRoot(page), 'Tasks page root');
  ...
  async saveKoekjes({ page, touch, eventBus, scene }) {
    ...
    if (await dialog.isVisible().catch(() => false)) {
      const createButton = dialog.getByRole('button', { name: 'Taak maken', exact: true });
      if (await createButton.count()) await tapLocator(touch, createButton, 'Tasks create button', { directClick: true, requireEnabled: true, holdAfterMs: readableResultHoldMs, clickTimeoutMs: 5000 });
    ...
    await expectSingle(page.getByText('Koekjes bakken', { exact: true }).first(), 'Created Koekjes bakken task');
