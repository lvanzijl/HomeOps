...
async function expectRedesignedTasksPage(page) {
  ...
  await expectSingle(root.getByLabel('Aandacht voor nu', { exact: true }), 'Tasks wide Today focus column');
  await expectSingle(root.getByLabel('Wat daarna komt', { exact: true }), 'Tasks planning column');
  await expectSingle(root.getByLabel('Later en terugkijken', { exact: true }), 'Tasks quiet queues column');
  await expectSingle(root.getByLabel('Taakplanning acties', { exact: true }), 'Tasks progressive-disclosure support actions');
  await expectSingle(root.getByText('Vandaag in beeld', { exact: true }), 'Tasks Today in-view summary label');
  return root;
}
...
const taskRecordingActions = Object.freeze({
  ...
  async completeZwemtas({ page, touch }) {
    await expectRedesignedTasksPage(page);
    const zwemtasCard = page.locator('.task-item').filter({ hasText: 'Zwemtas klaarzetten' }).first();
    await expectSingle(zwemtasCard, 'Zwemtas klaarzetten task card');
    ...
    const completedHistory = page.locator('details.task-completed-history').first();
    if (await completedHistory.count()) {
      const isOpen = await completedHistory.evaluate((element) => element.open === true).catch(() => false);
      if (!isOpen) await completedHistory.locator('summary').first().click({ force: true, timeout: 5000 });
      await page.waitForTimeout(250);
    }
    await expectSingle(page.locator('.task-item').filter({ hasText: 'Zwemtas klaarzetten' }).filter({ hasText: 'Afgerond' }).first(), 'Completed Zwemtas klaarzetten task');
    await holdReadable(page);
  },
});
...
const agendaRecordingActions = Object.freeze({
  ...
  async addFilmavond({ page, touch, eventBus, scene }, action) {
    ...
    const addEventButton = agendaRoot(page).locator('.agenda-command-main').getByRole('button', { name: 'Gebeurtenis toevoegen', exact: true });
    eventBus?.publish(recordingEventTypes.DialogOpenStarted, { sceneId: scene?.id, actionId: 'add-filmavond', dialogId: 'agenda-add-event' });
    await tapLocator(touch, addEventButton, 'Agenda header Add event button', action);
...
export const marketingPreviewV1Storyboard = Object.freeze({
  ...
  chapters: Object.freeze([
    ...
    { id: 'agenda', narrativeStep: 'planning', scenes: Object.freeze([scene({ id: 'agenda', fixture: 'visual-marketing-agenda', title: 'Agenda', subtitle: 'One rhythm for the whole family', purpose: 'Show month, week, list rhythm, then add one realistic plan from the header action.', narrativeRole: "Today's planning grows into a wider family rhythm with Filmavond.", emotionalTone: 'Confidence without rigidity', visualFocus: 'Month view, week view, list view, header add-event action, saved Filmavond event, and returned overview.', minimumDurationMs: 11000, preferredDurationMs: 14000, maximumDurationMs: 15000, transition: { type: 'dissolve' }, cameraPacing: 'Show Month and pause. Move to Week and pause. Move to List and pause. Keep event creation steady and readable.', touchGestures: ['Tap Month', 'Tap Week', 'Tap List', 'Tap header add event', 'Enter Filmavond', 'Save', 'Return to overview'], interactionSequence: ['Show Month', 'Pause', 'Show Week', 'Pause', 'Show List', 'Pause', 'Use the header add-event action for Filmavond', 'Save', 'Hold briefly so Filmavond is visible', 'Return to overview'], audioEvents: ['ChapterStarted', 'TouchStarted', 'TouchCompleted', 'ActionCompleted', 'save'], expectedFinalState: 'Agenda overview is visible with Filmavond added as a believable event.', directorNotes: 'Pause between views and after the save. The point is confidence across time scales.', timing: { initialHoldMs: 1300, viewHoldMs: 900, postSaveHoldMs: 1200, postSceneHoldMs: 700 }, actions: [agendaAction('show-month', 'Show Month view.', agendaRecordingActions.showMonth, { hesitationMs: 80, holdAfterMs: 250, durationMs: 220, directClick: true, clickTimeoutMs: 5000 }), action('pause-month', 'pause', 'Pause on Month view.', { holdMs: 1000 }), agendaAction('show-week', 'Show Week view.', agendaRecordingActions.showWeek, { hesitationMs: 80, holdAfterMs: 250, durationMs: 220, directClick: true, clickTimeoutMs: 5000 }), action('pause-week', 'pause', 'Pause on Week view.', { holdMs: 1000 }), agendaAction('show-list', 'Show List view.', agendaRecordingActions.showList, { hesitationMs: 80, holdAfterMs: 250, durationMs: 220, directClick: true, clickTimeoutMs: 5000 }), action('pause-list', 'pause', 'Pause on List view.', { holdMs: 1000 }), agendaAction('add-filmavond', 'Add one event named Filmavond.', agendaRecordingActions.addFilmavond, { hesitationMs: 80, holdAfterMs: 180, durationMs: 220, directClick: true, clickTimeoutMs: 5000, typingDelayMs: 0 }), agendaAction('save-filmavond', 'Save Filmavond.', agendaRecordingActions.saveFilmavond, { hesitationMs: 80, holdAfterMs: 300, durationMs: 220, locatorClickSteps: 1 }), action('hold-filmavond-visible', 'pause', 'Hold briefly so Filmavond is visible.', { holdMs: 1000 }), agendaAction('return-agenda-overview', 'Return to the agenda overview.', agendaRecordingActions.returnOverview, { hesitationMs: 80, holdAfterMs: 250, durationMs: 220, directClick: true, clickTimeoutMs: 5000 })] })]) },
    { id: 'tasks', narrativeStep: 'helping', scenes: Object.freeze([scene({ id: 'tasks', fixture: 'visual-marketing-tasks', title: 'Tasks', subtitle: 'Small jobs, shared rhythm', purpose: 'Show practical tasks added and completed without drama.', narrativeRole: 'Planning turns into helping each other.', emotionalTone: 'Confidence shifting into helpful relief', visualFocus: 'The redesigned desktop Tasks dashboard: Today summary, wide Vandaag focus column, quiet planning columns, progressive add-task dialog, progressive Koekjes bakken entry, and completion of Zwemtas klaarzetten.', minimumDurationMs: 8000, preferredDurationMs: 10000, maximumDurationMs: 11000, transition: { type: 'crossfade' }, cameraPacing: 'Hold on the cleaner Today-first dashboard first; move through the progressive add-task questions without rushing; return to the wide Today focus; center Zwemtas klaarzetten; pause longer after completion.', touchGestures: ['Tap add task', 'Enter Koekjes bakken', 'Tap through owner/date/extras progressively', 'Save', 'Tap completion affordance'], interactionSequence: ['Show Today summary and desktop task columns', 'Add Koekjes bakken through the progressive dialog', 'Wait for it to appear in the redesigned task card layout', 'Complete Zwemtas klaarzetten', 'Wait for completion animation'], audioEvents: ['TouchStarted', 'TouchCompleted', 'ActionCompleted', 'save', 'task completed'], expectedFinalState: 'The redesigned Tasks dashboard remains visible, the progressive Koekjes bakken entry has been shown, and Zwemtas klaarzetten has completed with visual state settled.', directorNotes: 'The Tasks scene should feel like the same marketing beat, but the cleaner Today-first layout must be obvious without narration. The completion animation must finish before transition; this resolves the swimming-lesson thread.', timing: { initialHoldMs: 1200, postAddHoldMs: 850, completionAnimationHoldMs: 1200 }, actions: [taskAction('open-add-task-dialog', 'Open the Add Task dialog and hold it readable.', taskRecordingActions.openAddDialog, { hesitationMs: 170, holdAfterMs: 1000, dialogReadableHoldMs: 1000 }), taskAction('enter-koekjes-bakken', 'Type Koekjes bakken at readable speed, move through the progressive questions, and review the compact summary.', taskRecordingActions.enterKoekjes, { hesitationMs: 170, holdAfterMs: 1000, typingDelayMs: 45, questionHoldMs: 700, summaryHoldMs: 1000 }), taskAction('save-koekjes-bakken', 'Save Koekjes bakken and hold the created task visible.', taskRecordingActions.saveKoekjes, { hesitationMs: 170, holdAfterMs: 1000 }), taskAction('complete-zwemtas', 'Complete Zwemtas klaarzetten.', taskRecordingActions.completeZwemtas, { hesitationMs: 190, holdAfterMs: 1000 }), action('wait-completion-animation', 'pause', 'Wait for the completion animation to finish.', { holdMs: 1200, resultHoldMs: 1200 })] })]) },
